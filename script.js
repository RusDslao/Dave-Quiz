  ...
  if (userAns == correcAns) {
    ...
    for (let i = 0; i < allOptions; i++) {
      if (option_list.children[i].textContent == correcAns) {
        // If there is an option which is matched to the correct answer
...
function startTimerLine(time) {
  ...
  function timer() {
    ...
    if (deviceType === "desktop" && time > 549) {
      // If desktop and time > 549
      clearInterval(counterLine); // Clear counterLine
    } else if (deviceType === "mobile" && time > 320) {
      // If mobile and time > 320
      clearInterval(counterLine); // Clear counterLine
    }
  }
}
